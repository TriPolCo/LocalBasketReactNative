...
export default function VegetablesScreen() {
  ...
  const { data: response, isLoading, isError } = useProducts("vegetables");
  ...
  // Dynamic heights incorporating device top safe area (notch/status bar)
  ...
  return (
    <View className="flex-1 bg-slate-50">
      {/* Top title bar with safe area padding */}
      <Animated.View
      ...
      >
        ...
        >
          <View>
            <Text className="text-2xl font-extrabold tracking-tight text-emerald-800">
              Fresh Vegetables
            </Text>
            <Text className="mt-0.5 text-xs text-slate-500">
              {products.length} products · delivered in 30 min
            </Text>
          </View>
          ...
          <View className="flex-row items-center gap-2">
            ...
            >
              <Ionicons name="search-outline" size={19} color="#047857" />
            </Pressable>
            ...
            <Pressable
              ...
              className="h-10 w-10 items-center justify-center rounded-full bg-emerald-50"
            >
              <Ionicons name="cart-outline" size={19} color="#047857" />
              {cartCount > 0 && (
                <View className="absolute -right-1 -top-1 h-4 min-w-[16px] items-center justify-center rounded-full bg-emerald-700 px-1">
      ...
      {/* Sort & Filter Bar positioned immediately below the header */}
      <Animated.View
        style={{
          position: "absolute",
          top: HEADER_HEIGHT,
      ...
      >
        <View className="h-full flex-row border-y border-slate-200 bg-white">
          ...
          >
            <Ionicons name="swap-vertical-outline" size={18} color="#047857" />
          ...
          >
            <Ionicons name="options-outline" size={18} color="#047857" />
            <Text className="ml-2 text-sm font-bold text-slate-800">Filter</Text>
          </Pressable>
        </View>
      </Animated.View>

      {/* Search overlay matching dynamic top safe bounds */}
      {searchVisible && (
        <>
          <Pressable
          ...
          >
            <View className="flex-row items-center rounded-2xl bg-slate-100 px-4 py-3">
              ...
              <TextInput
                ...
                placeholder="Search for tomatoes, spinach..."
          ...
          return (
            ...
            >
              <View>
                <Image
                  source={{ uri: imageUrl }}
                  className="h-36 w-full bg-emerald-50"
                  resizeMode="cover"
                />

                {discount > 0 && (
                  <View className="absolute left-2 top-2 rounded-md bg-emerald-700 px-2 py-1">
              ...
              <View className="p-3">
                ...
                {qtyInCart === 0 ? (
                  <Pressable
                    ...
                    className="mt-3 items-center rounded-md bg-green-800 border border-emerald-700 py-2"
                  >
                  ...
                  <View className="mt-3 flex-row items-center justify-between rounded-xl bg-emerald-700 py-1.5 px-3">
                    <Text className="text-xs font-extrabold text-white">
                      In Cart: {qtyInCart}
                    </Text>
                    <Pressable
      ...
      {cartCount > 0 && (
        <View 
          className="absolute bottom-0 left-0 right-0 flex-row items-center justify-between rounded-t-2xl bg-emerald-800 px-5 py-4 shadow-lg"
        ...
        >
          ...
          >
            <Text className="mr-1 text-sm font-extrabold text-emerald-800">
              View Cart
            </Text>
        ...
        <View className="flex-1 flex-row bg-black/40">
          ...
          <View className="h-full w-[88%] bg-white">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <FilterSection title="Vegetable Type">
                <FilterRow label="Leafy vegetables" />
                <FilterRow label="Root vegetables" />
                <FilterRow label="Fresh herbs" />
                <FilterRow label="Seasonal vegetables" />
              </FilterSection>

              <FilterSection title="Price Range">
                <FilterRow label="Under ₹30" />
                <FilterRow label="₹30 to ₹50" />
                <FilterRow label="₹50 to ₹100" />
                <FilterRow label="Above ₹100" />
              </FilterSection>
            </ScrollView>
            ...
            <View 
              ...
              style={{ paddingBottom: Math.max(insets.bottom, 16) }}
            >
              <Pressable className="mr-3 flex-1 items-center rounded-xl border border-emerald-700 py-3">
                <Text className="font-bold text-emerald-700">Clear All</Text>
              </Pressable>
